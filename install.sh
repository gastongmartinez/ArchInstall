...
while :
do
    read -p "Seleccione el disco de instalacion (Ej.:/dev/sda) o 0 para cancelar: " DISCO
    if [ -b ${DISCO} ];
    then
        break
...
while :
do
    read -p "Indique el tamaño de la particion EFI (Ej.: 600M): " EFI
    if [ "${EFI: -1}" != "M" ];
    then
        echo "La unidad de medida debe ser en MiB indicandolo con una 'M' al final del valor."
        continue
    fi
    if [ "${EFI::-1}" -ge 550 -a "${EFI::-1}" -le 650 ];
    then
        break
    else
...
while :
do
    read -p "Indique el tamaño de la particion ROOT (Ej.: 28G): " RT
    if [ "${RT: -1}" != "G" ]; 
    then
...
while :
do
    read -p "Indique el tamaño de la particion SWAP (Ej.: 10G): " SP
    if [ "${SP: -1}" != "G" ]; 
    then
        echo "La unidad de medida debe ser en GiB indicandolo con una 'G' al final del valor."
        continue
...
while :
do
    read -p "Indique el tamaño de la particion HOME (Ej.: 20G o 0 para el resto del disco): " HM
